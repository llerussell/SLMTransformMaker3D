function movingPoints = cpselect3d_v2(movingStack, fixedPoints)
% Lloyd Russell 2018


% get stuff ready
if iscell(fixedPoints)
    fixedPoints = vertcat(fixedPoints{:});
    numGalvoPositions = numel(fixedPoints);
else
    numGalvoPositions = 1;
end
fixedPoints = fixedPoints(:,1:3);
allPoints = [fixedPoints nan(size([fixedPoints]))];
zPlanes = unique(allPoints(:,3));
...
hold on
movingPointsPlot = plot3(nan, nan, nan, 'ro');
...
%         roiAx.XTick = [1 zeroSlice numSlices];
        xtix = roiAx.XTick;
        roiAx.XTickLabels = stackRange(xtix);
        sliderUpdate
        updateMainTitle
    end
    ...
    function updateROITrace
        ...
        roiAx.YLim = [min([0 ROItrace]) max(ROItrace)];
        sliceMarkerLine.YData = roiAx.YLim;
        for j = 1:numZPlanes
